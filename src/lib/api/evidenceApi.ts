...
export function evidenceValidationMessage(file: File) {
  ...
  if (file.size < 1 || file.size > MAX_EVIDENCE_FILE_BYTES)
    return "Choose one file no larger than 5 MiB.";
  return null;
}
export async function uploadEvidenceFile({
  ...
  file: File;
  subjectType: "repair" | "return_request";
  subjectId: string;
  purpose: "repair_intake" | "return";
}) {
