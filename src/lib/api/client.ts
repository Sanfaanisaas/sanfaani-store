...
async function requestBlob(
  ...
  try {
    ...
    if (!response.ok) {
      const parsed = (await readJson(response)) as ApiFailureEnvelope | null;
      const error = failureFrom(response.status, parsed, response);
      if (
        error.kind === "unauthorized" &&
        !options.skipRefresh &&
        !retried &&
        ...
        const refreshed = await getRefreshResult();
        if (refreshed && !joined.signal.aborted)
          return requestBlob(method, path, options, true);
        clearSession?.();
      }
      throw error;
    }
...
export const apiClient = {
  ...
  blob: (path: string, options?: Omit<ApiRequestOptions, "body">) =>
