...
export default function RepairTrackingEntry() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (trackingToken.trim())
      rememberTrackingToken(repairId, trackingToken.trim());
        ...
        <section className="rounded-xl border border-navy-900/10 bg-white p-8">
          <div className="mb-8 text-center">
            ...
            <p className="mt-2 text-sm text-mist">
              Enter the repair reference. If you are not signed in, add the
              one-time tracking credential received when the request was
              created.
            </p>
          </div>
          <form onSubmit={submit} className="space-y-4">
            ...
            >
              ...
              <span className="mt-1 block text-xs font-normal text-mist">
                Kept only in memory for this browser tab. It is never added to
                the link.
              </span>
            </label>
            <button
              ...
              className="flex w-full items-center justify-center rounded-lg bg-gold py-3 text-sm font-bold text-navy-900 disabled:opacity-50"
            >
              Track repair
            </button>
          </form>
