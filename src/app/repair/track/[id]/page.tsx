            ...
            {quote && (
              ...
              >
                ...
                <p className="mt-3 text-sm text-mist">
                  Issued {new Date(quote.issuedAt).toLocaleString()}. Estimated
                  duration: {quote.estimatedDays} day
                ...
                {quote.superseded && (
                  <p className="mt-3 rounded-xl bg-paper p-3 text-sm text-mist">
                    This version has been superseded
                    {quote.supersededByVersion
                      ? " by version " + quote.supersededByVersion
                      : ""}{" "}
                    and is read-only.
                  </p>
                )}
                <section className="mt-4 rounded-xl bg-paper p-3 text-sm text-mist">
                  <h3 className="font-semibold text-ink">
                    Payment requirement
                  </h3>
                  {quote.depositRequirement.required ? (
                    <p className="mt-1">
                      A deposit of {money(quote.depositRequirement.amount)} is
                      required before work begins. Payment status:{" "}
                      {quote.paymentState.status.replaceAll("_", " ")}.
                      Confirmed: {money(quote.paymentState.confirmedAmount)};
                      remaining: {money(quote.paymentState.remainingAmount)}.
                    </p>
                  ) : (
                    <p className="mt-1">
                      No deposit is currently required. Payment status:{" "}
                      {quote.paymentState.status.replaceAll("_", " ")}.
                    </p>
                  )}
                </section>
