  ...
  return (
    <main className="bg-paper">
      ...
      <div className="mx-auto max-w-4xl px-6 py-12">
        ...
        {cart.status === "loading" ||
        cart.status === "hydrating" ||
        cart.status === "merging" ? (
          <div className="mt-8">
            <LoadingState>Loading your cart…</LoadingState>
          </div>
        ) : cart.status === "error" ? (
          <div className="mt-8">
            <ErrorState message={cart.error ?? "Your cart is unavailable."} />
          </div>
        ) : items.length === 0 ? (
          <div className="mt-10">
          ...
          <div className="mt-8 grid grid-cols-1 gap-10 lg:grid-cols-[1fr_320px]">
            {cart.conflicts.length > 0 && (
              <div
                role="alert"
                className="lg:col-span-2 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm"
              >
                {cart.conflicts.map((conflict) => (
                  <p key={conflict.type + conflict.message}>
                    {conflict.message}
                  </p>
                ))}
              </div>
            ...
            {message && (
              <p role="status" className="lg:col-span-2 text-sm text-mist">
                {message}
              </p>
            )}
            <div className="space-y-4">
              {items.map((item) => (
                <div
                  key={item.variantId}
                  className="flex items-center gap-4 rounded-xl border border-navy-900/10 bg-white p-4"
                >
                  <div
                    className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-navy-900/4 text-2xl"
                    aria-hidden="true"
                  >
                  ...
                  <div className="flex-1">
                    ...
                    {item.priceChanged ? (
                      <p className="text-xs text-amber-700">
                        Price updated since it was added.
                      </p>
                    ) : null}
                  </div>
                  <div className="flex items-center rounded-full border border-navy-900/20">
                    <button
                      type="button"
                      disabled={Boolean(cart.pendingVariants[item.variantId])}
                      onClick={() =>
                        void changeQty(
                          item.variantId,
                          item.sku,
                  ...
                  <button
                    type="button"
                    disabled={Boolean(cart.pendingVariants[item.variantId])}
                    onClick={() => void remove(item.variantId, item.sku)}
                    className="text-xs text-mist hover:text-red-600"
                  >
                    Remove
                  </button>
                </div>
            ...
            <div className="h-fit rounded-xl border border-navy-900/10 bg-white p-6">
              ...
              <p className="mt-2 text-xs text-mist">
                Final totals, delivery and stock are confirmed at checkout.
              </p>
              <Link
                href={isAuthenticated ? "/checkout" : "/login?next=/checkout"}
                className="mt-6 block rounded-full bg-navy-900 px-6 py-3 text-center text-sm font-medium text-paper hover:bg-navy-800"
              >
                {isAuthenticated
                  ? "Proceed to checkout"
                  : "Sign in to checkout"}
